...
Audit remaining `Skeleton` convenience API gaps against `spine-cpp`, then decide whether clipping-aware bounds merits a separate geometry slice or whether the next best gap is attachment/bounds clipping support. Keep the same verification shape: focused tests first, then the full core parity gate.
