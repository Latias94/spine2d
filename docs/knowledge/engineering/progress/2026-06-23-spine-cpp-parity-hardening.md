...
- Full parity gate passed on 2026-06-23 after adding `TrackEntryHandle::set_animation` in commit `01d87a7`: `599 passed, 10 skipped`. Focused `track_entry_set_animation` nextest passed with `2 passed, 607 skipped`; Bevy examples, WGPU/Web checks, `cargo fmt --all --check`, and `git diff --check` also passed.
...
- U5 TrackEntry animation-setter commit `01d87a7` added `TrackEntryHandle::set_animation`, a safe Rust equivalent of C++ `TrackEntry::setAnimation(Animation&)` that validates the replacement animation but keeps existing entry timing/mix fields unchanged.
