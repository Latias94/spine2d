...
Continue `AnimationState` parity audit around C++ `computeHold`/timeline mode representation, remaining `AnimationStateData` / `TrackEntry` surface, and dead compatibility code before attempting any `interruptAlpha`-shape refactor. Keep the same verification shape: focused tests first, then the full core parity gate.
