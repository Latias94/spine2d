...
- Last verified:
  ...
  - `cargo nextest run -p spine2d --features json,binary,upstream-smoke --no-fail-fast --status-level fail` passed with `599 passed, 10 skipped` on 2026-06-23 after the TrackEntry animation-setter slice. `cargo check -p spine2d-bevy --examples`, `cargo check -p spine2d-wgpu -p spine2d-web`, `cargo fmt --all --check`, and `git diff --check` passed. `TrackEntryHandle::set_animation` now resolves the canonical animation from `AnimationStateData` instead of cloning the caller-supplied object, and the regression test mutates the source duration to prove the entry keeps the canonical data. `block v0.1.6` still emits the existing future-incompatibility warning.
...
- Done:
  ...
  - Added `TrackEntryHandle::set_animation` in commit `578b3bf`; this is the safe Rust equivalent of C++ `TrackEntry::setAnimation(Animation&)`, validating the replacement against the owning `AnimationStateData` and storing the canonical animation from that data while preserving existing timing/mix fields like the C++ pointer swap.
