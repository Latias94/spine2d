...
- Last verified:
  - `cargo nextest run -p spine2d --features json,binary,upstream-smoke track_entry_set_animation --no-fail-fast --status-level fail` passed with `2 passed, 607 skipped` on 2026-06-23 after adding `TrackEntryHandle::set_animation` in commit `01d87a7`.
  - `cargo nextest run -p spine2d --features json,binary,upstream-smoke --no-fail-fast --status-level fail` passed with `599 passed, 10 skipped` on 2026-06-23 after the TrackEntry animation-setter slice. `cargo check -p spine2d-bevy --examples`, `cargo check -p spine2d-wgpu -p spine2d-web`, `cargo fmt --all --check`, and `git diff --check` passed. `block v0.1.6` still emits the existing future-incompatibility warning.
...
- Done:
  ...
  - Added `TrackEntryHandle::set_animation` in commit `01d87a7`; this is the safe Rust equivalent of C++ `TrackEntry::setAnimation(Animation&)`, validating the replacement against the owning `AnimationStateData` while preserving existing timing/mix fields like the C++ pointer swap.
