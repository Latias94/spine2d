...
- Direct-assignment path: default mix, pair mix, empty-animation mix duration, `add_empty_animation` delay, `AnimationState::update` delta, IK mix values, and timeline alpha values propagate the exact `f32` shape used by the official path, including reverse/non-finite or nonpositive values where C++ does not validate.
