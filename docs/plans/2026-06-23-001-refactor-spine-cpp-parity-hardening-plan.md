...
- API parity path: public TrackEntry blend control is `mix_blend` / `set_mix_blend(MixBlend)` / `with_mix_blend(MixBlend)`, matching C++ `getMixBlend/setMixBlend`; public hold control is `hold_previous` / `set_hold_previous(bool)` / `with_hold_previous(bool)`, matching C++ `getHoldPrevious/setHoldPrevious`; public query/control helpers include `is_complete`, `was_applied`, `is_empty_animation`, `set_loop`, `with_looped`, `previous`, `next`, and `is_next_ready`, matching C++ `isComplete`, `wasApplied`, `isEmptyAnimation`, `setLoop`, `getPrevious`, `getNext`, and `isNextReady`; the old Rust-only additive public API does not compile.
