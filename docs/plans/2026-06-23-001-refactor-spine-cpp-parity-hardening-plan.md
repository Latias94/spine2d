...
- API parity path: public TrackEntry blend control is `mix_blend` / `set_mix_blend(MixBlend)` / `with_mix_blend(MixBlend)`, matching C++ `getMixBlend/setMixBlend`; public hold control is `hold_previous` / `set_hold_previous(bool)` / `with_hold_previous(bool)`, matching C++ `getHoldPrevious/setHoldPrevious`; the old Rust-only additive public API does not compile.
