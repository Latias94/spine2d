...
**Recent update:** `Animation::apply` now has a combined slider-and-draw-order regression on the public entry. `animation_apply_updates_slider_and_draw_order_in_out_mix` exercises the `MixFrom::Current` / `out=true` path on a fixture that contains both slider timelines and a draw-order timeline, verifying that the public entry resets the applied draw order to setup while preserving the unconstrained pose and still updating slider values. Verification passed on 2026-06-28: `cargo fmt --all -- --check` and focused nextest (`4 passed, 281 skipped`).
...
- API parity path: latest-tag public TrackEntry controls are `get_additive` / `set_additive(bool)` / `with_additive(bool)` and `get_mix_interpolation` / `set_mix_interpolation(MixInterpolation)` / `with_mix_interpolation(MixInterpolation)`, matching C++ `getAdditive/setAdditive` and `getMixInterpolation/setMixInterpolation`; stale-checkout `mix_blend`, `set_mix_blend`, `with_mix_blend`, `hold_previous`, `set_hold_previous`, and `with_hold_previous` do not compile. Public query/control helpers include `get_track_index`, `get_animation`, `get_loop`, `get_reverse`, `get_shortest_rotation`, `get_animation_start`, `get_animation_end`, `get_animation_last`, `get_delay`, `get_track_time`, `get_track_end`, `get_time_scale`, `get_mix_duration`, `get_mix_time`, `get_alpha`, `get_alpha_attachment_threshold`, `get_mix_attachment_threshold`, `get_mix_draw_order_threshold`, `get_event_threshold`, `get_animation_time`, `get_track_complete`, `is_complete`, `was_applied`, `is_empty_animation`, `set_loop`, Bevy `with_loop`, `get_previous`, `get_next`, `is_next_ready`, `get_mixing_from`, `get_mixing_to`, `get_animation_state`, `set_track_time`, and `set_mix_time`, matching the corresponding C++ `TrackEntry` getters, predicates, links, owner getter, and time setters; `AnimationState::set_animation` / `add_animation` now accept both name and `Animation` inputs through the internal `AnimationInput` adapter, `AnimationState::apply` returns `bool` like C++ `apply(Skeleton&)`; `AnimationState::get_current` and `get_data` cover C++ `getCurrent` and `getData`; `disable_queue` and `enable_queue` cover C++ `disableQueue` and `enableQueue`; `set_manual_track_entry_disposal`, `get_manual_track_entry_disposal`, and `dispose_track_entry` cover the official manual disposal controls; `get_tracks()` preserves sparse track slots like C++ `getTracks()`.
...
**Recent update:** Commit `776c7c1` hid the remaining internal model plumbing that had no official C++ surface: `SequenceDef`, `VertexWeight`, `MeshVertices`, and `TimelineKind` are now crate-private. The public API no longer exports those helper containers, but the runtime and parsers still use them internally for attachment parsing and timeline ordering. Verification passed on 2026-06-26: `cargo fmt --all -- --check`, `cargo check -p spine2d --features json,binary,upstream-smoke`, `cargo nextest run -p spine2d --features json,binary,upstream-smoke --no-fail-fast --status-level fail` (`656 passed, 2 skipped`), `cargo check -p spine2d --examples --features json,binary,upstream-smoke`, `cargo check -p spine2d-bevy --examples`, `cargo check -p spine2d-wgpu -p spine2d-web`, and `git diff --check`.

**Recent update:** Commit `72fba7e` added official-style attachment data accessors after the field-visibility cleanup. Region, mesh, point, path, bounding-box, and clipping attachment data now expose the C++ getter/setter surface for names and their public editable attachment state, while preserving Rust's owned data representation for sequences, vertices, and linked-mesh bookkeeping. Verification passed on 2026-06-26: `cargo fmt --all -- --check`, `cargo check -p spine2d --features json,binary,upstream-smoke`, `cargo nextest run -p spine2d --features json,binary,upstream-smoke --no-fail-fast --status-level fail` (`656 passed, 2 skipped`), `cargo check -p spine2d --examples --features json,binary,upstream-smoke`, `cargo check -p spine2d-bevy --examples`, `cargo check -p spine2d-wgpu -p spine2d-web`, and `git diff --check`.
...
**Recent update:** `Skeleton` bounds and update-cache inspection now use latest-tag C++ getter spelling. `bounds`, `bounds_with_clipping`, and `update_cache_items` were removed in favor of `get_bounds`, `get_bounds_with_clipping`, and `get_update_cache`, matching `getBounds(...)` and `getUpdateCache()`.
...
**Status:** In progress. The render oracle toolchain is now scenario-only on both sides: `scripts/spine_cpp_lite_render_oracle.cpp` and `spine2d/examples/render_dump.rs` accept command streams instead of the deleted single-animation `--anim` / `--time` mode. `scripts/record_oracle_render_goldens.py` and `scripts/render_parity_smoke.zsh` use the same scenario surface, render goldens were re-recorded, and focused render verification stayed green against the pinned `spine-ts-4.3.8` / `8e12b1250ab88c0f890849ea45aab80338cead63` baseline. The old one-shot `spine2d/examples/pose_dump.rs` example was also deleted, leaving `pose_dump_scenario` as the single Rust pose-dump surface for single-track, mixing, and multi-track oracle samples. `pose_dump_scenario` now also accepts `--entry-event-threshold` and `--entry-alpha-attachment-threshold`, so the Rust pose, Rust render, and C++ pose/render scenario CLIs expose the same TrackEntry threshold controls. The Rust-only inherent `Atlas::parse` wrapper was deleted as a breaking public-surface cleanup; the standard `FromStr` implementation remains the supported text atlas entry point, and repo callers now use `str::parse::<Atlas>()`. The Atlas reader now also preserves C++ `getRegions()` parse order, page format/index/texture path/filter/wrap enum surface, region index/rotate, UVs, post-rotation packed dimensions, and unknown region metadata instead of the older Rust `HashMap` region surface; Rust-only `AtlasPage.scale`, `Atlas::page(index)`, `Atlas::region(name)`, and public `AtlasRegion.width` / `height` surfaces were removed.
...
**Recent update:** `Atlas` page and region storage is now private like latest-tag C++ `_pages` / `_regions`. Public enumeration uses `get_pages()` and `get_regions()`, matching C++ `getPages()` / `getRegions()`, while `AtlasPage` fields remain public because the C++ page type exposes them directly.

**Recent update:** `AtlasRegion` fields are now private like latest-tag C++ `AtlasRegion` / `TextureRegion`, and public inspection goes through C++-style getters such as `get_name`, `get_page`, `get_index`, `get_x`, `get_y`, `get_offset_x`, `get_offset_y`, `get_packed_width`, `get_packed_height`, `get_original_width`, `get_original_height`, `get_rotate`, `get_degrees`, `get_names`, `get_values`, `get_u`, `get_v`, `get_u2`, `get_v2`, `get_region_width`, and `get_region_height`. The old Rust-only region `width` / `height` fields were deleted; parser internals now use the C++ packed-dimension fields directly before the official 90-degree post-UV swap.

**Recent update:** `EventData`, `Event`, and `EventTimeline` now use latest-tag C++-style getter/setter access instead of public Rust fields. The Rust model keeps `EventData` ownership sound with `Arc<EventData>` references from keyed `Event` values and an `EventSetupPose` value object for setup defaults, rather than copying C++'s self-referential `EventData::_setupPose` layout. JSON/Binary event loading now builds events through constructors and setters, Bevy/event tests use getters, and focused coverage records the C++ audio default rule for omitted JSON `volume` / `balance`.
