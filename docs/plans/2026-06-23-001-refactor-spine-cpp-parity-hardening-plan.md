...
- Event queue path: raw event collection matches C++ `EventTimeline::apply`, `queueEvents` owns animation start/end filtering and complete-before/after ordering, and negative `trackLastWrapped` remains signed instead of being normalized before event split decisions.
- API parity path: public TrackEntry blend control is `mix_blend` / `set_mix_blend(MixBlend)` / `with_mix_blend(MixBlend)`, matching C++ `getMixBlend/setMixBlend`; public hold control is `hold_previous` / `set_hold_previous(bool)` / `with_hold_previous(bool)`, matching C++ `getHoldPrevious/setHoldPrevious`; public query/control helpers include `is_complete`, `was_applied`, `is_empty_animation`, `set_loop`, `with_looped`, `previous`, `next`, `is_next_ready`, `mixing_from`, `mixing_to`, `set_track_time`, and `set_mix_time`, matching C++ `isComplete`, `wasApplied`, `isEmptyAnimation`, `setLoop`, `getPrevious`, `getNext`, `isNextReady`, `getMixingFrom`, `getMixingTo`, `setTrackTime`, and `setMixTime`; `AnimationState::apply` returns `bool` like C++ `apply(Skeleton&)`; `AnimationState::current` and `data` cover C++ `getCurrent` and `getData`; `disable_queue` and `enable_queue` cover C++ `disableQueue` and `enableQueue`; `set_manual_track_entry_disposal`, `manual_track_entry_disposal`, and `dispose_track_entry` cover the official manual disposal controls; `tracks()` preserves sparse track slots like C++ `getTracks()`; the old Rust-only additive public API does not compile.
