...
- API parity path: public TrackEntry blend control is `mix_blend` / `set_mix_blend(MixBlend)` / `with_mix_blend(MixBlend)`, matching C++ `getMixBlend/setMixBlend`; the old Rust-only additive public API does not compile.
