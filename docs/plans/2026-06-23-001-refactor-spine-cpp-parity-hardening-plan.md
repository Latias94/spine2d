...
**Recent update:** Commit `01d87a7` added `TrackEntryHandle::set_animation`, the safe Rust equivalent of C++ `TrackEntry::setAnimation(Animation&)`. The Rust API validates that the replacement animation belongs to the owning `AnimationStateData` while preserving existing entry timing and mix fields like the C++ pointer swap.
