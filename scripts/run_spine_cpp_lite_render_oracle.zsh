...
mkdir -p "${BUILD_DIR}"
OUT="${BUILD_DIR}/spine_cpp_lite_render_oracle"
...
NEEDS_BUILD=0
if [[ ! -x "${OUT}" || "${ROOT_DIR}/scripts/spine_cpp_lite_render_oracle.cpp" -nt "${OUT}" ]]; then
  NEEDS_BUILD=1
...
if [[ "${NEEDS_BUILD}" == "1" ]]; then
  clang++ "${ORACLE_CXXFLAGS[@]}" \
    ...
    -I"${SPINE_C_SRC}" \
    -I"${SPINE_CPP_INCLUDE}" \
    "${ROOT_DIR}/scripts/spine_cpp_lite_render_oracle.cpp" \
